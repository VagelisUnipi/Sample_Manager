# Sample Manager Ingestion Pipeline — Development Task List

**Project:** Sample Manager Data Pipeline (FOS + DKQ → S3 → Glue → Redshift)
**Owner:** Vaggelis Karaferis
**Reference architecture:** `prsm-sfn-dev-samplemanager-run`

---

## Phase 1 — Source Analysis & Schema Design

- [x] Analyse source system schema from Business Objects universe export (DKQ DFX file)
- [x] Identify and document all 16 source tables and their column structures
- [x] Produce DDL definitions (CREATE TABLE) for all 16 tables to serve as the data contract
- [x] Document pipeline architecture (triggers, S3 zones, Glue, Redshift, data flow)

---

## Phase 2 — Pipeline Configuration Layer

- [x] Design metadata-driven config pattern (one JSON config file per source table)
- [x] Create 16 Glue table config files for DKQ source (`DKQ_{TABLE}.json`)
...
- [x] Build `lims-config-router-lambda` — scans landed zone, reads config, builds Glue job arguments per file
- [x] Add support for `HasHeader` and `Delimiter` per-table config fields
- [x] Fix compound extension handling for `.csv.gz` files
- [x] Implement environment-variable-only configuration (no hardcoded paths — promotion-ready)
...
- [x] Build `lims_ingestion_job` — reads CSV, injects partition columns, writes Snappy Parquet, registers Glue catalog table
- [x] Wire `HAS_HEADER` and `DELIMITER` args into Spark reader
- [ ] Upload Glue job script to S3 scripts bucket
- [ ] Create and configure Glue job in AWS console (IAM role, worker type, connections)
- [ ] Validate job runs successfully against mock data

---

## Phase 5 — Testing

- [x] Create mock CSV.GZ test files for all 16 DKQ tables (referentially consistent sample data)
- [ ] Create mock CSV.GZ test files for all 16 FOS tables
- [ ] Upload DKQ mock files to S3 landed zone
...
- [ ] Validate pipeline with full historical load (not just mock data)
